...
def parsePrice():
    ...
    #r = requests.get('https://finance.yahoo.com/quote/'+stock+'?p='+ stock+'&.tsrc=fin-srch')
    try:
        r = requests.get('https://finance.yahoo.com/quote/BTC-INR?p=BTC-INR&.tsrc=fin-srch')
        soup = bs4.BeautifulSoup(r.text,'lxml')
...
while True:
    ...
    print("The Price of " + str(stock) + " at " + str(now) + " is: " + str(stkprice))
    stkprice = float(stkprice.replace(',',''))
    if g-stkprice > 2000 and x == 1:
        print("Sold")
